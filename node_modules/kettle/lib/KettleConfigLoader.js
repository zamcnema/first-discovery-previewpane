...
kettle.config.getNodeEnv = function (outerDefault) {
    var nodeEnv = fluid.get(process.argv, "3") || process.env.NODE_ENV || outerDefault;
    if (nodeEnv) {
        fluid.log("Loader running configuration name " + nodeEnv);
    } else {
        fluid.fail("No configuration specified in either 2nd command line argument, NODE_ENV or internal default");
    }
    return nodeEnv;
};

kettle.config.getConfigPath = function (outerDefault) {
    var configPath = fluid.get(process.argv, "2") || outerDefault;
    if (!configPath) {
...
kettle.config.initCLI = function () {
    return kettle.config.loadConfig({
        configName: kettle.config.getNodeEnv(),
        configPath: kettle.config.getConfigPath("%kettle/tests/configs")
    });
};
...
kettle.config.createDefaultsImpl = function (prefix, filePath) {
    ...
    kettle.config.loadSubConfigs(prefix, fullPath, configFile.mergeConfigs, gradeNames);
    kettle.config.loadSubConfigs(prefix, fullPath, configFile.loadConfigs);
    ...
    fluid.each(requires, function loadRequire(requireId) {
        ...
        try {
            ...
            e.message += " while trying to resolve require directive for " + requireId + " in config at path " + fullPath;
            throw (e);
        }
    });
    ...
    fluid.log("Created defaults for config type " + configFile.type + ": " + fluid.prettyPrintJSON(configFile.options));
...
kettle.config.createDefaults = function (options) {
    return kettle.config.createDefaultsImpl(fluid.module.resolvePath(options.configPath), options.configName + ".json");
};
